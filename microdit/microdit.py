...
class RectFlowWrapper(nn.Module):
    ...
    def forward(self, x, cond, mask):
        ...
        z_noise_t = (1 - texp) * x + texp * z_noise

        vtheta = self.model(z_noise_t, t, cond, mask)

        batchwise_mse = ((z_noise - x - vtheta) ** 2).mean(dim=list(range(1, len(x.shape))))

        return batchwise_mse.mean()

    @torch.no_grad()
    def sample(self, z_noise, cond, null_cond=None, sample_steps=50, cfg=2.0):
        b = z_noise.size(0)
        dt = 1.0 / sample_steps
        dt = torch.tensor([dt] * b).to(z_noise.device).view([b, *([1] * len(z.shape[1:]))])
        images = [z_noise]

        for i in range(sample_steps, 0, -1):
            t = i / sample_steps
            t = torch.tensor([t] * b).to(z_noise.device)

            vc = self.model(z_noise, t, cond)
            if null_cond is not None:
                vu = self.model(z_noise, t, null_cond)
                vc = vu + cfg * (vc - vu)
